...
class PublicationDistributionList(models.Model):
    ...
    def get_product_contract_count(self, product_id, contract_partner_id):
        self.env.cr.execute(SQL_CONTRACT_COUNT, (product_id, contract_partner_id))
        return self.env.cr.fetchone()[0]

    @api.model
    def get_product_contract_assigned_count(self, product_id, contract_partner_id):
        self.env.cr.execute(SQL_ASSIGNED_COUNT, (product_id, contract_partner_id))
        return self.env.cr.fetchone()[0]
    ...
    def _compute_name_address(self):
        ...
        for this in self:
            if not this.product_id or not this.partner_id:
                ...
                continue
            this.name = " - ".join([this.product_id.name, this.partner_id.name])
            if this.product_id.distribution_type == "email":
                this.contact_address = this.partner_id.email
            else:
                delivery_id = this.partner_id.address_get(["delivery"])["delivery"]
                this.contact_address = partner_model.browse(delivery_id).contact_address
    ...
    partner_id = fields.Many2one(
        comodel_name="res.partner", string="Receiving Partner", required=True
    )
    contract_partner_id = fields.Many2one(
        comodel_name="res.partner", string="Contract Partner", required=True
    )
    date_start = fields.Date(
        string="Date start", default=fields.Date.today(), required=True
    )
    date_end = fields.Date(string="Date end")
    copies = fields.Integer(string="Number of copies", default=1)
    name = fields.Char(compute="_compute_name_address", string="Name")
    contact_address = fields.Char(
        compute="_compute_name_address", string="Receiving address"
    )
    contract_count = fields.Integer(
        string="Quantity to distribute", compute="_compute_counts"
    )
    assigned_count = fields.Integer(
        string="Quantity already assigned", compute="_compute_counts"
    )
    available_count = fields.Integer(
        string="Quantity still available", compute="_compute_counts"
    )
    ...
    def write(self, vals):
        old_values = (
            set(self.mapped(lambda x: (x.product_id, x.contract_partner_id)))
